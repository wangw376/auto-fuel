...
def detect(opt):
    ...
    (save_dir / 'labels ' if (save_txt or save_txt_tidl) else save_dir).mkdir(parents=True,
                                                                              exist_ok=True)  # make dir 20240109
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            '''--------20240109 存原图----------'''
            src = im0.copy()
            filename = os.path.splitext(p.name)[0]
            src_name = filename + '.png'
            save_path2 = str(save_dir / src_name)
            '''--------------------------------'''

            save_path = str(save_dir / p.name)  # img.jpg
            # txt_path = str(save_dir / 'labels' / p.stem) + ('' if dataset.mode == 'image' else f'_{frame}')  # img.txt 20240109
            s += '%gx%g ' % img.shape[2:]  # print string
            gn = torch.tensor(im0.shape)[[1, 0, 1, 0]]  # normalization gain whwh

            '''------20240109更改txt保存路径-------'''
            txt_root = './parameters/pred'
            if not os.path.exists(txt_root):
                os.mkdir(txt_root)
            txt_name = '%05d' % frame + '.json'
            txt_path2 = os.path.join(txt_root, txt_name)
            # print('txt',txt_path2)
            '''--------------------------------'''
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(det[:, :6]):
                    ...
                    if save_txt:  # Write to file
                        ...
                        '''------------------20240116写入关键点到label-------------------'''
                        # kptsnum = len(keypoints)
                        linelist = []
                        # print(keypoints)
                        index = 3

                        for kip in range(8):
                            x_pt0, y_pt0, score = keypoints[index * kip], keypoints[index * kip + 1], keypoints[index * kip + 2]
                            line2 = [x_pt0, y_pt0, score]
                            linelist.append(line2)
                        np.savetxt(txt_path2, linelist, fmt='%0.8f', delimiter='\t')
                        '''-----------------------------------------------------'''

                    solution = opt.solution
                    intrinsic = opt.intrinsic
                    target_model = opt.target_model
                    ret_path = opt.ret_path

                    ret_path2 = ret_path + '/' + txt_name
                    # print('ret_path', ret_path2)

                    '''-------EPnP位姿估计-------'''
                    if os.path.exists(txt_path2):
                        subprocess.run([solution, target_model, intrinsic, txt_path2, ret_path2])

                    '''------读取位姿解算结果------'''
                    with open(ret_path2, 'r') as f:
                        data = f.read()
                        if len(data) != 0:
                            # 从文件中读取初始姿态参数
                            data_split = data.split()
                            x, y, z, rx0, ry0, rz0 = float(data_split[3]), float(data_split[4]), float(
                ...
                '''调用摄像头时存图'''
                # save_dir2 = str(save_dir / 'src_img')
                # if not os.path.exists(save_dir2):
                #     os.mkdir(save_dir2)

                vid_img_name = '%05d' % frame + '.jpg'
                save_path3 = str(save_dir / vid_img_name)
                # save_path4 = os.path.join(save_dir2, vid_img_name)

                cv2.imwrite(save_path3, im0)  # 存检测图
                # cv2.imwrite(save_path4, src)  # 存原图
            ...
            if save_img:
                if dataset.mode == 'image':
                    ...
                    # cv2.imwrite(save_path2, src)#存原图
...
if __name__ == '__main__':
    ...
    #EPnP输入参数
    parser.add_argument('--solution', type=str, default='./parameters/estimation_v20230525.exe')
    parser.add_argument('--target_model', type=str, default='./parameters/model/model.json')
    parser.add_argument('--intrinsic', type=str, default='./parameters/intrinsic/fuel_intr.json')
    parser.add_argument('--ret_path', type=str, default='./parameters/ret')
